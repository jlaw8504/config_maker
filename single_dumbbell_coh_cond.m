function single_dumbbell_coh_cond(filename,line_beads,final_loop_beads,cohesin_number)
...
%Generate desired number of cohesins evenly spaced around the chain but not
%in the end loops
%find diameter of end loops
cirum = final_loop_beads * mass_sep;
diameter_m = cirum/pi();
%set offset as 1.5 time loop diameter to prevent cohesin loading near end
%loops
loop_offset = diameter_m * 1.5;
coh_offsets = linspace(min(y_pos)+loop_offset,max(y_pos)-loop_offset...
    ,cohesin_number);
...
for k = 1:size(cond_list,1)
    fprintf(fid,'spring %d %d %.1g %.6g\n',cond_list(k,1),...
            cond_list(k,2), spring_rest, spring_const);
end
fprintf(fid,'}\n');
fclose(fid);
end
