...
if size(springs_condensin,1)>0
    ...
    for z = 1:size(mass_DNA,1)
        ...
        if max(mass_DNA(z,5) == springs_condensin(:,1)) == 0
            ...
            mass_DNA_unb(m,1:5) = mass_DNA(z,1:5);
...
for z = 1:size(condensin_A,1)
    ...
    elseif DNA_alpha(z,5) == 0.5
        % case 2 corresponds to when the alpha spring is inactive
        ...
        for n = 1:size(mass_DNA_unb) % loop through all the DNA beads
            if mass_DNA_unb(n,1) > binding_box_x(z,1) && mass_DNA_unb(n,1) < binding_box_x(z,2) &&...
                ...
                alpha_bind_test(m,1:5) = mass_DNA_unb(n,1:5);
    ...
    elseif DNA_alpha(z,5) ~= 0.5 && sep_dist <= thresh*mass_sep
        ...
        if sep_dist_AB1 < sep_dist_AB2
            DNA_beta_close(z,1:5) = DNA_beta1(z,1:5);
            DNA_beta_far(z,1:5) = DNA_beta2(z,1:5);
            condensin_B_close(z,1:5) = condensin_B1(z,1:5);
        else
            DNA_beta_close(z,1:5) = DNA_beta2(z,1:5);
            DNA_beta_far(z,1:5) = DNA_beta1(z,1:5);
            condensin_B_close(z,1:5) = condensin_B2(z,1:5);
        end
        ...
        for n = 1:size(mass_DNA_unb) % loop through all the DNA beads
            if mass_DNA_unb(n,1) > binding_box_x(z,1) && mass_DNA_unb(n,1) < binding_box_x(z,2) &&...
                ...
                % log the close beads to be tested
                beta_bind_test(m,1:5) = mass_DNA_unb(n,1:5);
